...
int print_file (int file_descriptor, off_t* offsets_table, off_t* lengths_of_lines, int number_of_lines)
{
    off_t lseek_result = lseek(file_descriptor, 0, SEEK_SET);
    if (lseek_result == -1)
    {
        perror("Lseek error\n");
        return -1;
    ...
    {
        lseek_result = lseek(file_descriptor, offsets_table[i], SEEK_SET);
        ...
        if (lengths_of_lines[i] > current_line_size)
        {
            char* tmp = NULL;
            tmp = (char*)realloc(tmp, lengths_of_lines[i] * sizeof(char));
            if (tmp == NULL){
            ...
            current_line = tmp;
            current_line_size = lengths_of_lines[i];
        }

        int read_write_result = read(file_descriptor, current_line, lengths_of_lines[i]);
        ...
        read_write_result = write(STDOUT_FILENO, current_line, lengths_of_lines[i]);
        if (read_write_result == -1)
        {
...
int search(int file_descriptor, off_t* offsets_table, off_t* lengths_of_lines, off_t number_of_lines)
{
    ...
    while(1){
        ...
        select_result = select(1, &descriptors, NULL, NULL, &timeout);
        ...
        if (select_result == 0)
        {
            read_write_result = write(STDOUT_FILENO, "Time is out\n\n", 13);
            if (read_write_result == -1)
            {
                perror("Can't print line for user\n");
                free(current_line);
            ...
            read_write_result = print_file(file_descriptor, offsets_table, lengths_of_lines, number_of_lines);
            if (read_write_result == -1)
            {
